...
Take the simplest case, $y$ on a single $x$ with an intercept. The only control is the constant, so $\tilde V_i = x_i - \bar x \equiv d_i$ and the weight is $w_i = d_i^2$, the squared distance from the mean of $x$. Squaring concentrates: a few points far from $\bar x$ can dominate the fit while most points near it contribute almost nothing.

The figures draw $w_i$ as the thickness of the ray from the sample centroid $(\bar x, \bar y)$ to each point, with each point's share of the total weight printed beside it.
...
The $10\%$ of points furthest from $\bar x$ hold $35\%$ of the weight; the $10\%$ closest hold $0.14\%$. An observation sitting exactly at $\bar x$ has zero weight.
