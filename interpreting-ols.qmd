...
The algebra leaves one question open. @eq-wavg and @eq-exact are statements about aggregates: the $d_i^2$-weighted average of the $s_i$ is $\hat\beta_{OLS}$ exactly, and the same weights on the $b_i$ give the value $\hat\beta_{OLS}$ is unbiased for. Neither says how close one $s_i$ is to its own $b_i$. A natural hope is that $s_i$ is a noisy but usable reading of unit $i$'s effect, so that sorting the $s_i$ sorts the units by effect size. The simulation tests that hope, and confirms the two aggregate identities in a finite sample. We draw $n = 2000$ units with heterogeneous slopes $b_i \sim N(1, 0.5^2)$ and regressor $x_i \sim N(0, 2^2)$, and set $y_i = b_i x_i + \varepsilon_i$ with $\varepsilon_i \sim N(0, 2^2)$. We then compare each $s_i$ with its own $b_i$, and the two weighted averages with `lm`.
...
The hope fails. The $s_i$ are wildly dispersed ($\text{sd}(s_i) > 100$) and barely correlated with the true $b_i$ — Pearson $0.02$, rank correlation $0.26$, the latter higher once the explosive tails near $\bar x$ are set aside. The identities hold. The leverage-weighted average of the $s_i$ is $\hat\beta_{OLS}$ exactly; that is @eq-wavg. The same leverage applied to the true $b_i$ gives $0.99$, the value $\hat\beta_{OLS}$ estimates in expectation (@eq-exact), and this sample's $\hat\beta_{OLS} = 0.95$ sits about two standard errors below it. That gap is sampling variation: @eq-exact fixes the target, not the realisation.

Is noise the reason $s_i$ misses $b_i$? Turn it off entirely, set $y_i = b_i x_i$, and ask again how well each $s_i$ tracks its own $b_i$.
...
The correlation stays at $0.14$ with no noise in the data at all. The reason is visible in @eq-decomp: even without the error term, $s_i$ contains $\bar x(b_i - \bar b)/d_i$, which depends on every unit's slope through $\bar b$ and on every unit's position through $\bar x$. A ray from the centroid is anchored at $(\bar x, \bar y)$, and $\bar y$ shifts when any other unit's slope changes. So $s_i$ is a function of the whole sample, not of unit $i$ alone, and heterogeneity in the $b_i$ makes it impossible for any single ray to isolate its own unit's effect. Noise through $(e_i - \bar e)/d_i$ inflates the variance of $s_i$ further, but the low correlation is already there without it.

So $s_i$ has no reading on its own. The $d_i^2$-weighted average of the $s_i$ does, and it is exact. That aggregate targets a $d_i x_i$-weighted average of the $b_i$, and the rest of the chapter takes that object apart.
...
The sign of a realised weight is not the end of the causal question. Borusyak and Hull (2023) make the distinction by moving from the sample identity above to the population regression estimand. Write

$$y_i = V_i\beta_i + Y_i(0), \qquad r_i = V_i - X_i'\lambda,$$

where $r_i$ is the residual from the *population* projection of treatment on a low-dimensional $X_i$. There are two different ways to make the baseline term $E[r_iY_i(0)]$ vanish. An outcome model assumes
...
The second condition does two jobs: treatment is conditionally mean-independent of the untreated outcome and the effect, and the controls correctly span its conditional mean. Correctly fitting $E[V_i\mid X_i]$ alone is not enough if assignment still depends on $\beta_i$ or $Y_i(0)$.
...
The $\psi_i$ can be negative. Under @eq-design-assignment, however, average each realised weight over possible assignments while holding $X_i$ and $\beta_i$ fixed:

$$\phi_i=E[\psi_i\mid X_i,\beta_i]
       =\operatorname{Var}(V_i\mid X_i,\beta_i)\ge 0.$$ {#eq-exante-weight}

Thus the *same population estimand* also satisfies $\beta=E[\phi_i\beta_i]/E[\phi_i]$, a convex representation that rules out a sign reversal when all effects share a sign. This is not a correction to OLS and it does not generally recover the ATE. The variance can depend on $\beta_i$, so units in one $X$-stratum need not receive equal weight; because $\beta_i$ is unobserved, the weights are not generally identified either. Full conditional unconfoundedness is one sufficient, but stronger than necessary, condition under which they reduce to the estimable $\operatorname{Var}(V_i\mid X_i)$.
