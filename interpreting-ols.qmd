...
1.  **OLS is not the ATE under heterogeneity.** $\hat\beta_{OLS}$ is not the plain average effect $\bar b = E[b_i]$. It is a weighted parameter whose weights are $d_i x_i = (x_i - \bar x)x_i$ --- signed, not squared --- and it equals the ATE only when the $b_i$ are uncorrelated with those weights.
