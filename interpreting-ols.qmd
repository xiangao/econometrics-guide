...
The individual weights also depend on where $V$ has its zero. Replace $V_i$ by $V_i + c$. The controls carry an intercept, so $\tilde V_i$ does not move and neither does the total $\sum_i \tilde V_i^2$; each weight becomes $\tilde V_i V_i + c\,\tilde V_i$. So a reported share of negative weights is partly a choice of origin rather than a fact about the design, and it should be quoted with the scale of $V$ stated.

Centering is not a free normalization either. Under @eq-het, writing $V_i = (V_i - c) + c$ turns $\beta_i V_i$ into $\beta_i (V_i - c) + \beta_i c$, and that second piece is a *heterogeneous* intercept which the common $X_i'\gamma$ cannot absorb unless $\beta_i$ is constant. Centering therefore changes the structural parameterization. It earns its place when the reference level means something, or when the specification allows heterogeneous intercepts.
...
1.  **One rule.** OLS weights each observation by $w_i = \tilde V_i V_i$, the regressor times what is left of it after the controls (@eq-weff). The weights sum to $\sum_i \tilde V_i^2 \ge 0$; individual weights need not be positive. Two conditions travel with that reading: the remainder in @eq-weff has to vanish, in conditional expectation or in probability, and the individual weights shift with the origin of $V$.
2.  **Cross-section.** With binary treatment and saturated strata the weight on a stratum is $n_x \hat D(x)(1 - \hat D(x))$ (@eq-angrist): overlap gets the weight, and a stratum with no comparison gets none. Sorted by treatment status instead, the same weights put $1 - \rho$ on the ATT and $\rho$ on the ATU (@eq-reversed) — the ATE's own shares, reversed. Both qualifications from the body survive into the summary: the ATT/ATU reading is exact when the group means are linear in the propensity score, and the reversed shares need equal propensity variance across the two groups.
