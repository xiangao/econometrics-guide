...
The weights in @eq-weff are $w_i = \tilde V_i V_i$, and the denominator that normalises them is written $\sum_i \tilde V_i^2$. Why are those the same number, and why does the identity not carry down to the individual terms?
...
because $\tilde V$ is orthogonal to the fitted part of $V$: split $V_i = \hat V_i + \tilde V_i$ and the cross term $\sum_i \tilde V_i \hat V_i$ vanishes. So the weights sum to a sum of squares and $\sum_i w_i \ge 0$ always. A single $w_i$ is under no such constraint — it is negative whenever $\tilde V_i$ and $V_i$ have opposite signs. Non-negativity of the total says nothing about the terms.
