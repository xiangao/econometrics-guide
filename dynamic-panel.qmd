...
walk), and the level equation lets time-invariant regressors enter, which
differencing removes altogether.  That they appear is not the same as their
coefficients being identified.  A time-invariant regressor has no within
variation, so its own lagged differences cannot instrument it.  Identification
comes from the level moments, and it requires the regressor to be uncorrelated
with the unit effect $C_i$, or an external instrument for it.  For a
time-invariant regressor correlated with $C_i$ and lacking such an instrument,
system GMM reports a coefficient that is not identified.  A practical warning:
