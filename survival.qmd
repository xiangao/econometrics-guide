...
belongs before any estimator.  Censoring must be **independent**, or
noninformative: being censored may not carry information about when the event
would have happened.  What that requires is not the same for every estimator, and
the difference matters.

Kaplan-Meier estimates a *marginal* survival curve and adjusts for nothing, so it
needs censoring independent of the event time marginally, in the population the
curve describes.  Independence conditional on covariates does not rescue it.  If
sicker patients are censored earlier, the unadjusted curve is biased even when
censoring is independent within every covariate stratum.

The Cox partial likelihood conditions on the covariates in the model, so there
the weaker conditional version is enough: given those covariates, units censored
at a given time must be neither more nor less likely to experience the event than
comparable units still under observation.

When censoring depends on covariates and a marginal curve is the target, name the
estimand and make the adjustment explicit.  Stratify, standardize over the
covariate distribution, or weight by the inverse probability of remaining
uncensored.
...
relevant variables into the model, which weakens what the conditional estimators
have to assume, and then to argue explicitly why the censoring that remains is
...
Agreement in direction is then substantive evidence that the covariate acts the
same way under both, not an identity delivered by the parameterization.  If
either model is misspecified, nothing forces even the signs to match.
...
is 0 unless there is an event, then it turns 1.  The likelihood then takes the
same algebraic form as a sequence of Bernoulli trials, and that is what makes the
logistic-regression trick work.  The reason is a factorization, not independence.
A person contributes the probability of surviving each period, and of failing in
the last one if an event is observed; the chain rule writes that as a product of
one-period *conditional* probabilities, each equal to $h_{ij}$ or $1-h_{ij}$ for
individual $i$ at time $t_j$.  Rows from the same person are not free of one
another.  They share covariates and history, and a later row exists only because
the earlier ones were survived.  The distinction leaves the point estimates
alone, which is why the trick is standard, but it governs the covariance: cluster
on the person, or fit a frailty model, when unobserved heterogeneity leaves a
person's periods dependent given the model (Singer and Willett, 1993, has more
details).
