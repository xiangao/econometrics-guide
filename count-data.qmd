...
$$
 {\rm Var}_{QML} [\hat \beta_{QML}] = (\sum_{i=1}^N \mu_i X_i X_i')^{-1} (\sum_{i=1}^N \omega_i X_i X_i') (\sum_{i=1}^N \mu_i X_i X_i')^{-1}
$$
...
$$
{\rm E} [Y | X, \tau]=\mu \tau = e^{X' \beta} \cdot \tau
$$
...
\frac{{\rm Pr} (y | X)}{1- \exp (- \mu)}
$$

where the last equality holds for the Poisson case, since ${\rm Pr}
(y=0|X)=e^{-\mu}$; for the negative binomial, ${\rm Pr} (y=0|X)=
...
$$ \hat{ \frac{\partial p}{\partial x}} \approx \bar p (1- \bar p) \hat \beta $$
