...
- **Heteroscedasticity of known form**, e.g. $\omega_t^2 = \exp({\bf z}_t' \gamma)$: regress $\log(\hat u_t^2)$ on $\bf z_t$ to estimate $\gamma$, then set $\hat\omega_t^2 = \exp({\bf z}_t' \hat\gamma)$ and apply WLS as above.
- **AR(1) serial correlation**, $u_t = \rho u_{t-1} + \epsilon_t$: regress $\hat u_t$ on $\hat u_{t-1}$ to get $\hat\rho$, then apply the Cochrane-Orcutt (or Prais-Winsten) transformation -- quasi-differencing each variable as $\tilde y_t = y_t - \hat\rho y_{t-1}$ (and similarly for $X$) -- so that the transformed error is approximately i.i.d.
