...
author: "Research Computing Services, Harvard Business School"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
Then it's natural to estimate $\theta$ by picking the $\theta$ that maximize this joint distribution function.    A parameter vector $\hat \bf \theta$ at which the
...
Here I use R's "gmm" library which makes things easy.  It expects two
arguments: "g" and "x", which correponds to $u$ and $W$ here.  The
moment condition is $E(W u) = 0$ in this example.  $W$ is the
instrument, and $u$ is the residual from regressing $y$ on endogenous
$X$.
...
$$
\begin{aligned}
\mbox{Pr}(y_t^0 \geq 0) & = \mbox{Pr} ({\bf X}_t \beta +u_t \geq 0) \\
...
$$
\begin{aligned}
y_t^0 & = {\bf X}_t \beta +u_t, \quad u_t \sim NID(0, \sigma^2) \\
...
$$
\begin{aligned}
\mbox{Pr}(y_t= 0) & = \mbox{Pr} (y_t^0 \leq 0) = \mbox{Pr} ({\bf X}_t \beta +u_t \leq 0) \\
...
y_t = 2 \quad \mbox{if} \ y_t^0 \geq \gamma_2.
$$
...
tricky to calculate, since and independent variable, if appearing in
...
There must be exclusion condition to make the selection
...
a hurdle model assumes that the two processes are separate; there is
no selection.
...
For the same model as in equation the equation above, the fixed-effect
...
equation the equation above from the following equation:
...
![Various hazard functions from Davidson and MacKinnon](davidson2.png)
...
likelihood of independent binomial process (Singer and Willett, 1993, has more details).
