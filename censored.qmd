...
$$
y_t={\bf X}_t \beta + \rho\sigma v_t +e_t
$$
...
$$
y_t={\bf X}_t \beta + \rho\sigma \frac{\phi({\bf W}_t \hat
\gamma)}{\Phi({\bf W}_t \hat \gamma)} + e_t
$$

Or,
$$
y_t={\bf X}_t \beta + \rho\sigma \hat \lambda_t + e_t
$$
...
$$
y_t = z_t ({\bf X}_t \beta_1 + u_{1t}) + (1-z_t)({\bf X}_t \beta_0 + u_{0t}),
\qquad z_t^0 = {\bf W}_t \gamma + v_t,
$$
...
treatment effect ${\bf X}_t(\beta_1-\beta_0)$ then varies with $\bf X_t$ instead
