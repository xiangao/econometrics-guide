...
is endogenous. Endogeneity of $x_2$ means $x_2$ has a non-orthogonal
(non-zero) projection onto $u$ -- not that $x_2$ is literally parallel
to $u$, which would be a knife-edge special case -- while $x_1$ remains
perpendicular to $u$. Suppose $z$ is a vector that is perpendicular to
$u$ (exogeneity: $z$ is uncorrelated with the error) but has a
non-orthogonal projection onto $x$, in particular onto its endogenous
part $x_2$ (relevance: $z$ actually predicts the endogenous regressor).
