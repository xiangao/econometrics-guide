...
Additional valid instruments that carry independent information can improve
asymptotic precision under a suitable weighting scheme.  The gain is not
automatic: a redundant instrument adds nothing, a weak one need not help, and a
large number of instruments worsens finite-sample behaviour even when every one
of them is valid.  The other advantage of over-identification is that
over-identification tests can be done to test the adequacy of instruments.
...
With extra instruments, Sargan's test lets us test the joint compatibility of
the overidentifying restrictions.  As stressed above, that is a test of the set
as a whole and not a verification of orthogonality: passing it leaves the
instruments untested in every direction they share.
...
coefficient equals $\Pi (\beta - \beta_0)$, which is zero exactly when
$\beta=\beta_0$ provided $\Pi$ has full column rank.  Without that rank
condition the test stays valid and weak-instrument robust, but it cannot
distinguish $\beta_0$ from an alternative differing in a direction $\Pi$
annihilates:
