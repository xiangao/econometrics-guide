...
This is essentially to get the part of $\bf X$ that is correlated with $\bf Z$.  Or to say, to project $\bf X$ on to $\bf Z$.
...
\sigma^2 (X'P_ZX)^{-1}
$$
where $\hat \sigma^2 ={\bf \hat u' \hat u} / N$.  This is the
...
Suppose the simplest case: $y=\beta_0 + \beta_1 x + u$.  Split $x$ into the part
...
Now let $z$ be a vector perpendicular to $u$ (exogeneity: $z$ is uncorrelated
with the error) with a non-zero projection onto $x$ (relevance: $z$ actually
predicts the regressor).  Project both $y$ and $x$ onto $z$, and write $\hat
y_z$ and $\hat x_z$ for those two projections.  The instrumental variables
estimator is the ratio of their lengths,

$$ \hat \beta_{IV} = \frac{\hat y_z}{\hat x_z} = \frac{z'y}{z'x}, $$

since $z'y = \beta_1 z'x + z'u = \beta_1 z'x$.  Because the two right triangles
involved are similar, this is the same number you would get from the *infeasible*
regression of $y$ on the exogenous part alone: that regression returns
$$ \frac{x^{\perp\prime}y}{x^{\perp\prime}x^{\perp}} = \beta_1 \frac{x^{\perp\prime}x}{x^{\perp\prime}x^{\perp}} + \frac{x^{\perp\prime}u}{x^{\perp\prime}x^{\perp}} = \beta_1, $$
...
where (normalizing ${\rm Var}(Z_i)=1$, which is harmless since any
