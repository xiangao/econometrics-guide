...
$$ f({\bf y} \mid {\bf X}, \beta, \sigma) = (2 \pi \sigma^2)^{-n/2} \exp\left(-  \frac{({\bf y- X \beta})'({\bf y- X \beta})}{2 \sigma^2}\right)$$

The log-likelihood function is

$$ l({\bf y, \beta, \sigma}) = -\frac{n}{2} \log(2 \pi) - \frac{n}{2} \log(\sigma^2) - \frac{1}{2 \sigma^2} \bf (y-X \beta)'(y-X \beta) $$
...
$$
{\bf \sqrt{n}(\hat \theta - \theta)} \xrightarrow{d} N(\bf 0, I^{-1}(\theta)),
$$
where $\bf I(\theta)$ below is the *per-observation* information.
Equivalently, in finite-sample terms,
$$
{\bf \hat \theta} \approx N(\bf \theta, I_n^{-1}(\theta)), \qquad I_n(\theta) = n\, I(\theta),
$$
with $\bf I_n(\theta)$ the *full-sample* information.  The $\sqrt{n}$
...
$\bf I(\theta)$ is defined using the single-observation log-likelihood
contribution $l_t$ (so that the full-sample log-likelihood is
$l=\sum_{t=1}^n l_t$):

$$
{\bf I(\theta)}=E[(\frac{\partial l_t}{\partial
...
If $\bf \hat \theta$ is the MLE estimator of $\theta$ and $\bf V$ denotes its
*asymptotic* variance matrix -- the variance of the limiting distribution of
$\sqrt n (\hat \theta - \theta)$, not of $\bf \hat \theta$ itself, the two
differing by a factor of $n$ exactly as with the information matrix above --
then
$$
\sqrt n (\hat \theta -\theta) \rightarrow^d N(\bf 0, V), \qquad \bf V = I^{-1}(\theta).
$$

If $\bf \tilde V$ denotes the asymptotic variance matrix of any other *regular*
consistent, asymptotically normal estimator, then $\bf \tilde V- V$ is a positive
...
Even if this assumption does not hold, we still have asymptotic normality for $\bf \hat \beta$, so the tests below remain valid asymptotically.
...
$$
{\bf \frac{\hat u' \hat u}{\sigma^2}} \sim \chi^2(n-k)
$$
...
$\rm H_0: \bf R\theta - r = 0$ after maximum likelihood needs only the asymptotic
...
$\bf I_n(\theta) = n\,I(\theta)$ for the full-sample information.
...
$$
W = ({\bf R}\hat\theta_u - {\bf r})' \bigl[{\bf R\, I}_n^{-1}(\hat\theta_u)\,{\bf R}'\bigr]^{-1} ({\bf R}\hat\theta_u - {\bf r}) \xrightarrow{d} \chi^2(q).
$$
...
$$
LM = s(\hat\theta_r)'\, {\bf I}_n^{-1}(\hat\theta_r)\, s(\hat\theta_r) \xrightarrow{d} \chi^2(q).
$$
...
$$ \sqrt{T} ({\bf b_T - \beta}) = [(1/T) {\sum_{t=1}^T \bf x_t
  x_t'}]^{-1} [(1/{\sqrt{T}}) {\sum_{t=1}^{T} \bf x_t u_t}] $$

The first term converges in probability to some constant.  The second
term is $\sqrt{T}$ times the sample mean of the vector $\bf x_t u_t$,
...
where $Q = {\rm plim} \, (1/T)\sum_{t=1}^T {\bf x_t x_t'} = {\rm E}[{\bf x_t x_t'}]$, and $S$ can be estimated by
...
$$\hat \Sigma_{NW} = [\sum_{t=1}^T x_t x_t']^{-1} [ \sum_{t=1}^T \hat u_t^2 x_t x_t' +  \sum_{v=1}^{q} {(1- \frac{v}{q+1})} \sum_{t=v+1}^T (x_t \hat u_{t, T} \hat u_{t-v, T} x_{t-v}' + x_{t-v} \hat u_{t-v, T} \hat u_{t, T} x_{t}') ] [\sum_{t=1}^T x_t x_t']^{-1} $$
