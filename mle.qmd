...
Binary response is a different case, and it is worth being exact about why.  If
$P(Y=1 \mid X) = F(X'\beta)$ is correct, then the whole conditional distribution
is correct: a Bernoulli law has nothing left to specify once its mean is fixed.
Logit and probit are therefore correctly specified maximum likelihood here, not
quasi-MLE.  The latent-variable story is a representation rather than an extra
assumption, and a different latent error distribution generating the same
$F(X'\beta)$ is observationally equivalent.  Keep the quasi-MLE label for the
Poisson case above, where the criterion is used without the full conditional
distribution being correct.
...
Even if this assumption does not hold, we still have asymptotic normality for ${\symbf{\hat \beta}}$, so the tests below remain valid asymptotically -- provided the covariance estimator is valid for whatever heteroskedasticity or dependence is actually present.  The model-based $\sigma^2 ({\symbf{X'X}})^{-1}$ is not: it encodes the homoskedastic, serially uncorrelated structure of @eq-mle-11.  Drop normality, keep that formula, and the tests are no longer asymptotically valid.  Use a sandwich estimator under heteroskedasticity and a HAC estimator under serial correlation.
