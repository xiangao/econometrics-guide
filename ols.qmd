...
![](ols.png){fig-alt="A parallelogram drawn in perspective represents the plane spanned by the two regressors. Arrows labelled x-one and x-two point outward from a common origin inside the plane. The vector y rises above the plane, the vector y-hat lies within it, and a short segment labelled u joins y-hat up to y, meeting the plane at a right angle."}
...
This is a moment condition, and it is what GMM estimators are built on.  It is a separate assumption from getting the functional form right: a correctly specified linear equation can still have $E({\symbf{Xu}}) \neq 0$, which is exactly the endogeneity problem of a later chapter.

### Consistency

Consistency does not need strict exogeneity. A much weaker condition suffices:
$$
{{\mathrm{E}}} (u_t|X_t)=0,
$$ {#eq-ols-22}

This condition is much weaker since it only assumes that the mean of current error term does not depend on the current predictors.  Even a model with lagged dependent variable can easily satisfy this condition.  This condition is called **contemporaneous exogeneity**.  It is the weakest of the
three conditional-mean conditions worth keeping apart.  It is sufficient for
consistency but not necessary: what consistency actually requires is the moment
condition at the foot of the list below.
...
- **Strict exogeneity**, $E[u_t \mid X_1, \dots, X_n]=0$: the error in every
  ...
  violates.
- **Predeterminedness**, $E[u_t \mid \mathcal F_t]=0$, where
  $\mathcal F_t = \sigma(X_1, \dots, X_t)$ is the regressor history through period
  $t$ -- equivalently, $X_t$ is orthogonal to the
...
- **Contemporaneous orthogonality**, $E[X_t u_t]=0$ -- mere zero correlation. This
  ...
  rank condition and a law of large numbers -- which for dependent
  data needs the weak-dependence conditions of the [MLE chapter](mle.qmd) -- it gives
...
is a more restrictive requirement, not a less restrictive one.  The ladder needs the
information sets to be nested, which is why $\mathcal F_t$ is defined above as
regressor history:
$\sigma(X_t) \subseteq \sigma(X_1,\dots,X_t) \subseteq \sigma(X_1,\dots,X_n)$.
Widen $\mathcal F_t$ to include past errors or other series and strict exogeneity no
longer implies predeterminedness, because then neither information set contains the
other.  By the law of iterated expectations,
$$
E[u_t \mid X_t] = E \bigl[ E[u_t \mid \mathcal F_t] \;\big|\; X_t \bigr] = 0,
$$ {#eq-ols-23}
