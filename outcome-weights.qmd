...
Second, the weights still reproduce each estimate exactly and sum to zero overall. They part on the other constraint from @eq-omega, $\sum_i \omega_i V_i = 1$: for a binary treatment that constraint is the treated weights summing to $+1$ (and, with the zero total, controls to $-1$). AIPW keeps it exactly; the partially-linear estimator sums to $\pm 0.997$. That shortfall has a direct reading. If the treated weights sum to $a$ and the controls to $-a$, then $\hat\tau = a\,(\bar y^{\,\omega}_{\text{treated}} - \bar y^{\,\omega}_{\text{control}})$, so the estimate is the weighted contrast scaled by $a$: at $a = 0.997$ the two differ by $0.3\%$. Whether that is an understatement depends on treating the normalized contrast as the target, which nothing here establishes — better read as a normalization discrepancy, and as a measure of how far a cross-fitted residual maker sits from a projection. The reason OLS and AIPW hit $1$ exactly *here* is a property of the fitted smoother, not a general feature of AIPW. Both use a residual maker that is a projection reproducing constants, which gives $\tilde{\boldsymbol V}'\boldsymbol V = \tilde{\boldsymbol V}'\tilde{\boldsymbol V}$ and hence a ratio of one. Fit the outcome model with a learner lacking that property and the AIPW sums move off $\pm 1$ as well. A cross-fitted smoother is not idempotent, so the identity only holds approximately, and the sum reports how far off it is.
...
Negative weights in staggered designs are usually described at the level of two-by-two comparisons, and the description needs care, because two different decompositions get run together. Goodman-Bacon (2021) writes TWFE as a weighted average of all two-by-two DiD comparisons, and those comparison weights are *non-negative*: they are built from group shares and treatment-variance terms and they sum to one. What goes wrong there is the estimand, not the weight. A comparison that uses an already-treated cohort as its control subtracts that cohort's own treatment effect whenever effects move over time, so what it contributes is not an ATT. That is the forbidden comparison. De Chaisemartin and D'Haultfœuille (2020) decompose TWFE differently, into a weighted sum of *treatment-effect* parameters, and there some weights genuinely are negative. Both are effect-weight readings. The outcome-weight reading goes one level further down, to the individual $y_{it}$.
...
The rank is at most the number of treated cohorts, and equals it generically. This ties the chapter's difference-in-differences material together. A block design has one adoption date, so its weight matrix is rank one. The plain $2 \times 2$ and synthetic difference-in-differences are the same object at two settings of the margins, uniform in the first and fitted in the second. Staggered adoption raises the rank. Once a cell's weight no longer factors into a unit part times a time part, a treated cell can enter negatively. In the block case it cannot, since every treated post-period cell gets $u_i v_t = 1/(N_1 T_1) > 0$. Rank one is not what does that work, though. An outer product $\boldsymbol u \boldsymbol v'$ has negative entries as soon as either margin has mixed signs, so rank one constrains the *structure* of $\boldsymbol\Omega$ and not the signs of its entries. What rules out a negative treated cell here is the block design's particular sign pattern, both margins uniform and positive.
...
7. **Rank counts the cohorts.** As an $N \times T$ matrix, the outcome weights of a block design have rank one and factor into a unit part times a time part (@eq-outcome-weights-13). The plain $2 \times 2$ and synthetic difference-in-differences differ only in whether those margins are uniform or fitted. Staggered adoption raises the rank, to at most the number of treated cohorts and generically to exactly that. Rank one is not itself what keeps treated cells positive: in the block design that follows from both margins being uniform and positive, and a rank-one matrix with mixed-sign margins has negative entries.
8. **Both margins carry correction, and the effective count reports its price.** Weighting can offset differing trends only under a model in which trends vary by unit; then either margin helps alone and both together help most. What it costs is effective sample size: by @eq-ess the count reaches $N_0$ only under uniform weights, so concentrating weight lowers the effective count. The optimisation is trading fit against that concentration, and @eq-sdid-penalty shows synthetic difference-in-differences charging itself for exactly that.
9. **Chosen weights are read differently from implied ones.** Weights fitted on $\boldsymbol Y$ make $\hat\tau$ non-linear in $\boldsymbol Y$, so inference must account for their estimation, and a good pre-period match is what the design optimised rather than a test it passed. Fitting on a noisy pre-period statistic is a close cousin of a pretest rather than the thing itself: Roth (2022) studies conditioning on a pass/fail test, and what the two share is that pre-period noise gets to select the analysis. Matching on pre-period *levels* invites regression to the mean (Daw and Hatfield 2018); fitting with an intercept, so only trajectories need to match, avoids that case.
