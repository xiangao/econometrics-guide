...
The companion chapter [Interpreting OLS: Effect Weights](interpreting-ols.qmd) treats the OLS coefficient as a weighted average of unobserved effects and asks whose effect $\hat\beta$ estimates. We can read the same Frisch--Waugh--Lovell identity as a weighting of observed outcomes. This second reading asks which rows move $\hat\beta$ and in which direction. Unlike the effect weights, these outcome weights can be calculated from the data and extended beyond OLS.
